...
export default function NotesCard({
  ...
  return (
    ...
    >
      {/* Glow Effect */}
      <div className="absolute -inset-0.5 bg-linear-to-r from-amber-400/20 to-orange-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl pointer-events-none rounded-3xl"></div>
      <button
        ...
        <div className="flex items-center gap-4">
          <div
            className={`h-12 w-12 rounded-2xl flex items-center justify-center text-2xl transition-colors shadow-lg ${
              notes.length > 0
                ? "bg-linear-to-br from-amber-400 to-orange-500 text-white shadow-amber-900/20 ring-1 ring-white/10"
          ...
          <div className="text-left">
            ...
            <p
              className={`text-xs font-medium ${
                notes.length > 0 ? "text-yellow-200/70" : "text-slate-500"
              }`}
            >
              {notes.length > 0
